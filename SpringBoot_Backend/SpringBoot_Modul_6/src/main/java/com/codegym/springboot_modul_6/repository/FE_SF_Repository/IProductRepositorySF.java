...
public interface IProductRepositorySF extends JpaRepository<ProductSF, Long> {
    ...
    @Query(value = "select * from btob_database.product right join product_detail on product.id = product_detail.product_id where product.category = ?1 group by product.name order by product_detail.price asc" , nativeQuery = true)
    Page<ProductSF> getAllProductPriceAsc(String category, PageRequest of);

    @Query(value = "select * from btob_database.product right join product_detail on product.id = product_detail.product_id where product.category = ?1 group by product.name order by product_detail.price desc" , nativeQuery = true)
